...
<style>
    ...
    .app-title { font-size: 42px !important; font-weight: 900 !important; color: #6CE0AC !important; text-align: center; margin-bottom: 8px; }
    
    /* Cards */
    .overview-card {
        background: rgba(255,255,255,0.15); backdrop-filter: blur(12px); border-radius: 20px; padding: 26px 16px;
        text-align: center; border: 1.5px solid rgba(255,255,255,0.25); box-shadow: 0 10px 30px rgba(0,0,0,0.4);
        height: 155px; transition: 0.3s;
    }
    ...
    /* Goal Section */
    .goal-container {
        background: rgba(255,255,255,0.15); border-radius: 24px; padding: 32px; text-align: center;
    ...
    /* RECOMMENDATION CARD — Yeh wahi hai jo tu chahta tha */
    .rec-card {
        margin-top: 24px; padding: 22px; border-radius: 20px; font-size: 18px; font-weight: 700;
    ...
    .stButton>button {
        background: linear-gradient(90deg, #0ea5e9, #6366f1); color: white; border: none; border-radius: 50px;
        padding: 16px; font-weight: 700; width: 100%; box-shadow: 0 8px 25px rgba(0,0,0,0.4);
    }
</style>
...
with col1: if st.button("Overview", use_container_width=True): st.session_state.page = "overview"
with col2: if st.button("AI Insights", use_container_width=True): st.session_state.page = "insights"
with col3: if st.button("Visuals", use_container_width=True): st.session_state.page = "visuals"
if "page" not in st.session_state: st.session_state.page = "overview"

# ==============================
# MAIN PAGE — SABSE PERFECT WALA
# ==============================
if st.session_state.page == "overview":
    ...
    <div class='goal-container'>
        ...
        <div class='rec-card {
            'rec-red' if goal_progress < 50 else
            'rec-orange' if goal_progress < 90 else
            'rec-green'
        }'>
            {
            'Goal bohot peeche hai!<br><b>Abhi Rs 10,000/month zyada bachaen</b>' if goal_progress < 50 else
            'Achha ja raha hai!<br><b>Auto-save on karen aur focus rakhen</b>' if goal_progress < 90 else
            'Mubarak ho! Goal qareeb hai<br><b>Ab naya goal set karen</b>'
            }
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown("<br><br>", unsafe_allow_html=True)
    st.markdown("<p style='text-align:center; background:rgba(255,255,255,0.12); padding:20px; border-radius:18px; color:#E0E7FF; font-size:17px;'>Inputs change karen? → Sidebar mein 'Analyze / Predict' dabana na bhoolen!</p>", unsafe_allow_html=True)
