...
if not st.session_state.logged_in:
    ...
    with col2:
        ...
        if st.button("Login", use_container_width=True):
            if username == "admin" and password == "123":  # Apna password change kar lena
...
# ========================= CSS — SUPER PREMIUM + GLOWING BUTTONS =========================
st.markdown("""
<style>
    ...
    /* GLOWING NAV BUTTONS */
    .nav-container { text-align:center; margin:50px 0 60px 0; }
    .glow-btn {
    ...
    /* CARDS, GOAL, RECOMMENDATION — (same premium style jo pehle tha)
...
<div class='nav-container'>
    <button class='glow-btn active'>Overview</button>
    <button class='glow-btn'>AI Insights</button>
    <button class='glow-btn'>Visuals</button>
</div>
""", unsafe_allow_html=True)
...
# ========================= CHART (CLEAR LABELS) =========================
st.markdown("<h3 style='text-align:center; color:white; margin-top:60px;'>Financial Overview</h3>", unsafe_allow_html=True)
chart_data = pd.DataFrame({"Category": ["Income","Expenses","Savings","Investments"], "Amount": [monthly_income, monthly_expenses, current_savings, current_investments]})
fig = px.bar(chart_data, x="Category", y="Amount", color="Category", text=chart_data["Amount"].apply(lambda x: f"Rs {x:,}"),
...
fig.update_layout(showlegend=False, plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', height=500, font=dict(color="white"))
st.plotly_chart(fig, use_container_width=True)

st.markdown("<div style='text-align:center; background:rgba(255,255,255,0.12); padding:20px; border-radius:18px; color:#E0E7FF; font-size:17px; margin-top:30px;'>Tip: Inputs change karen? → 'Analyze / Predict' zaroor dabana!</div>", unsafe_allow_html=True)
st.markdown("---")
